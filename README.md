...
`PAVO_EVE_APPROVE`（**default: false** = COMMENT 固定。プロンプトインジェクションで不正な
APPROVE を作らせないための既定。`true` で承認を解禁） / `PAVO_EVE_IGNORE` / `PAVO_EVE_DISABLED`
...
webhook を Active off に戻すと、Pavo（eve）のレビューが完全に停止する（kill switch は他に
Vercel env `PAVO_EVE_DISABLED=true`）。
