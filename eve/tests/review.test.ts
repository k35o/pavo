...
import { parsePatchLines } from '../agent/lib/patch.ts';
import {
  buildReviewBody,
  decideEvent,
  partitionComments,
  renderCommentBody,
} from '../agent/lib/review.ts';
import type { Finding, ReviewPolicy } from '../agent/lib/review.ts';

const CONFIG: ReviewPolicy = {
  ignore: ['*.lock'],
...
const comment = (overrides: Partial<Finding> = {}): Finding => ({
  path: 'src/a.ts',
  line: 2,
  side: 'RIGHT',
  severity: 'warning',
...
test('decideEvent: 🔴/🟡 があると approve でも COMMENT、approve=false なら常に COMMENT', () => {
  ...
  assert.equal(
    decideEvent('approve', [], [{ comment: comment({ severity: 'critical' }), reason: 'below min_severity' }], CONFIG),
    'COMMENT',
  );
